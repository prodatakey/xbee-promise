...
module.exports = function xbeePromiseLibrary(options) {
  ...
  function _sendFramePromiseResponse(frame, timeoutMs, responseFrameType) {
    ...
    return new Bluebird(function(resolve, reject) {
      // Set the frame ID 
      frame.id = xbeeStream.nextFrameId();
      ...
      completion = function resolverCallback(receivedFrame) {
      ...
      completions.on(responseFrameType, completion);

      // Send the frame to the network
      xbeeStream.write(frame);
    })
    // Return our promise with a timeout
    .timeout(timeoutMs)
    .finally(function() {
      // clean up: remove the completion after the promise is complete (for any reason)
      completions.removeListener(responseFrameType, completion);
    });
  }
  ...
  function discoverNodes(callback) {
    ...
    return localCommand('NT')
      .then(function(nt) {
        ...
        completions.on('ND', nodeInfoListener);
        ...
        return localCommand('ND')
          ...
          .finally(function() {
            completions.removeListener('ND', nodeInfoListener);
          });
      });
  }
  ...
  parambulator({
    streamSource: 'required$, function$',
    required$: 'module',
  ...
  function initStreams() {
    ...
    var writeStream = xbee_api.createWriteStream(xbeeOptions);
    var readStream = xbee_api.createReadStream(xbeeOptions);

    readStream.on('readable', function() {
      // Notify all completions of the new frame
      var frame = this.read();
      completions.emit(frame.type, frame);
    });
    options.streamSource(function(stream) {
